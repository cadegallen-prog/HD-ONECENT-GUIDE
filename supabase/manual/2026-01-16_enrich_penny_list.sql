-- MANUAL ONE-OFF (NOT A SUPABASE MIGRATION)
--
-- This file is intentionally NOT in `supabase/migrations/` because it is not schema/RLS,
-- it is a one-time data backfill for `penny_item_enrichment`.
--
-- Keeping it under `supabase/migrations/` causes `supabase db push` / `supabase migration list`
-- to warn and skip it because the filename does not match Supabase's required migration pattern.
--
