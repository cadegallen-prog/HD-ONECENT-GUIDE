-- Migration: 022_consume_enrichment_rpc_skip_retail_price.sql
-- Purpose: Stop copying retail_price from enrichment_staging into "Penny List".
--
-- Why:
-- - enrichment_staging is populated from upstream "penny-items" scan data where `retail_price`
--   can reflect store/region-specific clearance context (and can drift from HomeDepot.com).
-- - Once written, the SerpApi enrichment flow is "fill blanks only" by default, so a wrong
--   non-null retail_price can persist and degrade UX.
--
-- Policy:
-- - Keep using staging for canonical name, image, UPC, and URLs.
-- - Leave "Penny List".retail_price unset so the SerpApi enrichment job can fill it later.
...
COMMENT ON FUNCTION consume_enrichment_for_penny_item IS 'Atomically consume a staging row to enrich a Penny List item. Matches by SKU (priority) or internet_number, applies merge rules (never degrade), deletes the staging row, and intentionally does not copy retail_price from staging.';
