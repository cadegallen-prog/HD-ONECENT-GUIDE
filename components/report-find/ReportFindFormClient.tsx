...
import { AlertCircle, CheckCircle2, Package, Calendar, Info, Pencil } from "lucide-react"
        ...
        <div>
          ...
          {skuError ? (
            ...
            <p id="sku-hint" className="mt-1 text-xs text-[var(--text-muted)]">
              Enter the 6 or 10-digit SKU from the shelf tag or Home Depot app. Do not use Store SO
              SKU, Internet #, or UPC (receipts show UPC, not SKU).
            </p>
          )}
          {skuWarning && (
        ...
        {result && (
          <div
            className={`flex items-start gap-3 p-4 rounded-lg border border-[var(--border-default)] border-l-4 bg-[var(--bg-elevated)] ${
              result.success ? "border-l-[var(--status-success)]" : "border-l-[var(--status-error)]"
            }`}
          >
            {result.success ? (
