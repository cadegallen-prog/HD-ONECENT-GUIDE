...
### 1. **Home Depot Product Image Scraper**
   - **Why:** Visual browsing is 10x more engaging than text-only lists (Pinterest effect)
   - **What:** Fetch product images from homedepot.com by SKU, cache in Vercel Blob Storage
   - **Done means:**
     - `lib/scrape-hd-image.ts` created with HD scraper (og:image meta tag)
     - `lib/image-cache.ts` created for Vercel Blob caching
     - Rate limiting: 1 req/sec max
     - Fallback to placeholder on error
     - Images auto-fetch during build/on-demand for new submissions
   - **Files:** NEW: `lib/scrape-hd-image.ts`, `lib/image-cache.ts` | MODIFY: `lib/fetch-penny-data.ts`
