...
2) Follow the read order listed there (STATE → BACKLOG → CONTRACT/DECISION_RIGHTS → CONSTRAINTS/FOUNDATION_CONTRACT/GUARDRAILS → latest SESSION_LOG → CONTEXT when making product calls).
3) Use [USAGE.md](USAGE.md) only for copy/paste prompts. All other guidance is anchored from the root README.

---

## Minimal reminders
- Default: no new dependencies; no orphan one-off files. If you add, prune an obsolete item and log it.
- Run gates on meaningful changes: npm run lint, npm run build, npm run test:unit, npm run test:e2e. Record results in SESSION_LOG.
