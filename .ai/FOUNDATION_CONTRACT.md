...
## Palette Refresh (permission granted for future initiative)

- A planned “Palette Refresh” may update global tokens and component colors if it keeps WCAG **AA minimum** (target AAA) across themes.
- Changes must ship with before/after screenshots for key routes in both light and dark modes.
- Coordinate updates with `lint:colors` baseline refresh when intentional color shifts occur.
