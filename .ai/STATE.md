...
- **Autonomous automation (Jan 15):** Implemented Dependabot weekly updates, scheduled Snyk daily scans, and added weekly Supabase backups (4-week rolling window). Enforced Python tooling via `ruff` + `.pre-commit-config.yaml` and updated VS Code Python settings; added `.ai/SENTRY_ALERTS_MANUAL.md` with steps to tune Sentry. Verified via `reports/verification/2026-01-15T11-11-41/summary.md`.
- **Skimlinks script + test guard (Jan 15):** Injected the Skimlinks `<script src="https://s.skimresources.com/js/297422X1784909.skimlinks.js" />` before `</body>` while teaching `scripts/ai-verify.ts` to run with `SKIMLINKS_DISABLED=1` so the build/e2e bundles stay console-clean. Verified via `reports/verification/2026-01-15T10-52-07/summary.md`.
