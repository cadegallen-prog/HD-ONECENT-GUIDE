...
test.describe("Penny List Redesign - Visual Verification", () => {
  ...
  test("Hot Right Now cards include Home Depot link", async ({ page }) => {
    await page.setViewportSize(VIEWPORTS.mobile)
    await page.goto("/penny-list")

    const hotSection = page.locator('section[aria-labelledby="hot-items-heading"]')
    await expect(hotSection).toBeVisible({ timeout: 10000 })

    const firstHotCard = hotSection.locator('[role="link"]').first()
    await expect(firstHotCard).toBeVisible({ timeout: 10000 })

    const homeDepotLink = firstHotCard.locator('a[aria-label^="Open Home Depot page"]')
    await expect(homeDepotLink).toBeVisible()
    await expect(homeDepotLink).toHaveAttribute("href", /homedepot\.com/)
  })
  ...
  test("Card view interactions - State breakdown sheet opens on Line B tap", async ({ page }) => {
    ...
    const stateButtons = page.locator(
      'button:has-text("states"), button:has-text("State data unavailable")'
    )
