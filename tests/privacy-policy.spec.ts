...
test("privacy policy loads and contains required disclosures", async ({ page }) => {
  ...
  await expect(body).not.toContainText(/Rakuten/i)
  await expect(body).not.toContainText(/affiliate/i)
  await expect(body).not.toContainText(/referral compensation/i)
  await expect(body).not.toContainText(/paypal/i)
  await expect(body).not.toContainText(/donation/i)
