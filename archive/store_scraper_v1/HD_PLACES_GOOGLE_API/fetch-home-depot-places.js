...
function normalizePlace(place) {
  const prev = previousById.get(place.id) || null
  let street = ""
  let city = ""
  let state = ""
  let zip = ""
  ...
  const prevNumber = prev && typeof prev.number === "string" ? prev.number.trim() : ""
  const numericPrevNumber = prevNumber && /^\d{1,5}$/.test(prevNumber) ? prevNumber : ""

  const prevName = prev && typeof prev.name === "string" ? prev.name.trim() : ""
  const baseName = place.displayName?.text || prevName || "Home Depot"

  return {
    id: place.id || place.name || `${street}-${city}`,
    // Prefer a previously stored numeric store number if present; otherwise leave blank for later enrichment
    number: numericPrevNumber || "",
    name: baseName,
    address: street,
    city,
    state,
