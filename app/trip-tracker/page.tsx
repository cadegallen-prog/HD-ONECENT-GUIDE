...
export default function TripTrackerPage() {
  ...
  const upcomingTrips = trips.filter(t => !t.completed && new Date(t.date) >= new Date(new Date().toISOString().split('T')[0]))
  const pastTrips = trips.filter(t => t.completed || new Date(t.date) < new Date(new Date().toISOString().split('T')[0]))
          ...
          <div className="max-w-6xl mx-auto">
                  ...
                  <div className="p-6 space-y-6">
                    ...
                    {!selectedTemplate && (
                      <div>
                        ...
                        <div className="text-center my-4 text-sm text-muted-foreground">or start from scratch</div>
                      </div>
                    )}
                    ...
                    <div>
                      <label className="block text-sm font-medium mb-2 text-foreground">Stores to Visit</label>
                    ...
                    <div>
                      <label className="block text-sm font-medium mb-2 text-foreground">Checklist</label>
                      <div className="space-y-2 mb-2">
                        {formData.checklist.map((item, i) => (
                          <div key={i} className="flex items-center gap-2 bg-accent/50 px-3 py-2 rounded-lg">
              ...
              <div className="mb-8">
                ...
                <div className="space-y-4">
                  {upcomingTrips.map(trip => (
                    <TripCard key={trip.id} trip={trip} onToggle={toggleComplete} onDelete={deleteTrip} />
                  ))}
                </div>
              </div>
            )}
            ...
            {trips.length === 0 && (
              <div className="text-center py-16">
                ...
                <h3 className="text-xl font-heading font-semibold mb-2 text-foreground">No trips planned yet</h3>
                <p className="text-muted-foreground mb-6">Start planning your first penny hunting trip!</p>
        ...
        <div className="flex-1">
          ...
          <div className="flex flex-wrap gap-4 text-sm text-muted-foreground">
            ...
            <div className="flex items-center gap-1">
              ...
              {trip.stores.filter(s => s).length} store{trip.stores.filter(s => s).length !== 1 ? 's' : ''}
            </div>
          </div>
        </div>
      ...
      {trip.stores.filter(s => s).length > 0 && (
        <div className="mb-3">
          ...
          <div className="flex flex-wrap gap-2">
            {trip.stores.filter(s => s).map((store, i) => (
