...
const faqFlow = [
  {
    ...
    id: "step-1-basics",
    title: "Step 1: Basics",
    summary: "Start here for definitions, lifecycle context, and realistic expectations.",
  },
  {
    key: "Verification",
    id: "step-2-verification",
    title: "Step 2: Verification",
    summary: "Learn how to confirm signal quality and avoid bad assumptions.",
  },
  {
    key: "Checkout & Policy",
    id: "step-3-checkout-policy",
    title: "Step 3: Checkout & Policy",
    summary: "Understand register outcomes, refusal scenarios, and policy friction points.",
  },
  {
    key: "Etiquette & Community",
    id: "step-4-etiquette-community",
    title: "Step 4: Etiquette & Community",
    summary: "Finish with behavior standards that protect long-term access.",
  },
]
...
export default function FAQPage() {
  ...
  const groupedFaqs = faqFlow.map((flow) => ({
    ...flow,
    items: faqs.filter((faq) => faq.category === flow.key),
  }))
          ...
          <Prose variant="guide">
            ...
            <ol className="mb-8 space-y-1">
              {groupedFaqs.map((group) => (
                <li key={group.id}>
                  <a href={`#${group.id}`} className="text-[var(--cta-primary)] underline">
                    {group.title}
                  </a>{" "}
                  <span className="text-[var(--text-secondary)]">- {group.summary}</span>
                </li>
              ))}
            </ol>
            ...
            <div className="space-y-10">
              {groupedFaqs.map((group) => (
                <section key={group.id} id={group.id}>
                  <h2>{group.title}</h2>
                  <p className="mb-5 text-[var(--text-secondary)]">{group.summary}</p>
                  <div className="space-y-5">
                    {group.items.map((faq) => (
                      <article
                        key={faq.question}
                        className="border-b border-[var(--border-default)] pb-5 last:border-b-0 last:pb-0"
                      >
                        <h3 className="mb-2 text-[var(--text-primary)]">{faq.question}</h3>
